...
static void test_parse_array() {
    ...
    for (i = 0; i < 4; ++i) {
        lept_value* tmp = lept_get_array_element(&v, i);
        EXPECT_EQ_INT(LEPT_ARRAY, lept_get_type(tmp));
        EXPECT_EQ_SIZE_T(i, lept_get_array_size(tmp));
        for (j = 0; j < i; ++j) {
            EXPECT_EQ_INT(LEPT_NUMBER, lept_get_type(lept_get_array_element(tmp, j)));
            EXPECT_EQ_DOUBLE(j, lept_get_number(lept_get_array_element(tmp, j)));
        }
    }
    lept_free(&v);
}
